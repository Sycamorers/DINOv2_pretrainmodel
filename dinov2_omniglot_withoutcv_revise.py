...
class DINOClassifier:
    ...
    def pick_dinov2_version(self, version):
        ...
        if version == 'vits14':
            ...
            return torch.hub.load('facebookresearch/dinov2', 'dinov2_vitb14').cuda()
        elif version == 'vitll4':
    ...
    #     with torch.no_grad():
    #         for dataloader in [self.train_dataloader, self.test_dataloader, self.val_dataloader]:
    ...
    def extract_embeddings(self):
        """
        Function to extract embeddings from the DINO v2 model for training, validation, and testing
        """
        ...
        with torch.no_grad():
            for phase in ["train", "val", "test"]:
                for images, target in tqdm(dataloaders[phase]):
                    images = images.to(self.device)
                    embedding = self.dinov2(images)
                    embeddings[phase].append(embedding)
                    targets[phase].append(target)

        for phase in ["train", "val", "test"]:
            embeddings[phase] = torch.cat(embeddings[phase], dim=0).cpu().numpy()
            targets[phase] = torch.cat(targets[phase], dim=0).cpu().numpy()
    ...
    def train(self, X_train, y_train, X_val, y_val):
        ...
        Args:
            ...
            X_val: embeddings from DINO-v2 for validation
            y_val: true target for validation
        ...
        for epoch in range(self.epochs):
            ...
            val_acc.append(val_correct / y_val.size(0))

            self.model.train()
            print(f'Epoch {epoch+1}/{self.epochs}, Loss: {loss.item()}, Train Accuracy: {acc}, Test Accuracy: {val_acc[-1]}')
...
dinov2_version = 'vitls4' # Specify which dino-v2 model to use (vitls4, vitlb4, vitl14, vitg14)
...
embeddings, targets = classifier.extract_embeddings()
# Split the data into training and testing sets
epoch_acc, val_acc = classifier.train(embeddings["train"], targets["train"], embeddings["val"], targets["val"])
